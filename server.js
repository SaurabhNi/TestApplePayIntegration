			...
			request(options, function (error, response, body) {
			  if (error) {
			  	...
			  	if(body.state = 'approved') {
					...
					res.writeHead(302, {'Location':"https://android-ec-nvp-server.herokuapp.com/paypalPaymentSuccess?PAYID="+body.id});
					res.end();
...
router.get('/.well-known/assetlinks.json', function(req, res, next) {
	res.send('[{"relation": ["delegate_permission/common.handle_all_urls"],"target": {"namespace": "android_app","package_name": "com.example.paypalcustomtabdemo","sha256_cert_fingerprints":["AC:C4:A4:4E:4D:9C:01:C4:C6:6A:D6:4C:B6:6E:50:FE:8A:BC:2B:E4:C9:87:53:07:9E:CD:C0:EC:4C:14:82:B6"]}}]');
});
