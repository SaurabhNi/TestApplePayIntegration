					   ...
					   request(options, function (error, response, body) {
						 if (error) {
						   ...
						   console.log("Data User Id Token : ",user_id_token);
							...
							res.render("index",{token: user_id_token, client_token:body.client_token});
						 }
						});
...
// });
				...
				// 	console.log (text);
				// 	res.render("index",{token: body.id_token});
				//   });
				 });
			  // });
			//});
...
function getAccessToken(cb) {
	...
	var token  = configuration.CLIENT_ID+":"+configuration.SECRET;
	var encodedKey = new Buffer(token).toString('base64');
	var payload = "grant_type=client_credentials&Content-Type=application%2Fx-www-form-urlencoded&response_type=token&return_authn_schemes=true&ignoreCache=true";
	    // headers = {
		// 	'authorization': "Basic "+encodedKey,
		...
		function getUserIdToken(cb) {
	
			var url = configuration.ACCESS_TOKEN_URL;
			var token  = configuration.CLIENT_ID+":"+configuration.SECRET;
			var	encodedKey = new Buffer(token).toString('base64');
			var	payload = "grant_type=client_credentials&response_type=id_token";
				...
				function getUserIdTokenForReturnUser(cb) {
	
					var url = configuration.ACCESS_TOKEN_URL;
					var token  = configuration.CLIENT_ID+":"+configuration.SECRET;
					var	encodedKey = new Buffer(token).toString('base64');
					var	payload = "grant_type=client_credentials&response_type=id_token&target_customer_id=VLmBOtPgOb";
		...
		getAccessToken(function(data) {
            ...
            //     //}
                ...
                "purchase_units": [
					...
					// 		 "shiiping_preference":"SET_PROVIDED_ADDRESS"
					// 	  }
					// 	}
					//   }
            },
